...
if( ! class_exists( 'avia_form' ) )
{
	...
	/**
		...
		function send()
		{
			...
			if(empty($new_post['subject']) && !empty($this->form_params['subject'])) $new_post['subject'] = $this->form_params['subject'];
			$subject 	= empty($new_post['subject']) ? __("New Message", 'avia_framework') . " (".__('sent by contact form at','avia_framework')." ".$myblogname.")"  : $new_post['subject'];
